...
export const resolveUserSession = async (authUser: MinimalAuthUser): Promise<EmployeeUser> => {
    // 1. Ensure user exists in public.users, upserting from auth data.
    ...
    // 2. Fetch the complete user profile from the `users` table with joins.
    ...
    if (fetchError) {
        console.error('Error fetching user profile from users table:', fetchError);
        throw fetchError;
    }

    // 3. Construct and return the EmployeeUser object.
    return {
        id: userData.id,
        employeeNumber: userData.employee_number,
        name: userData.name,
        departmentId: userData.department_id,
        departmentName: userData.department?.name || null,
        positionId: userData.position_id,
        positionName: userData.position?.name || null,
...
export const getInboxItems = async (): Promise<InboxItem[]> => { const { data, error } = await supabase.from('inbox_items').select('*'); if (error) throw error; return data.map((d: any) => ({ ...d, fileUrl: supabase.storage.from('inbox').getPublicUrl(d.file_path).data.publicUrl })); };
export const addInboxItem = async (item: Partial<InboxItem>): Promise<InboxItem> => { const { data, error } = await supabase.from('inbox_items').insert(item).select().single(); if (error) throw error; return data; };
export const updateInboxItem = async (id: string, updates: Partial<InboxItem>): Promise<void> => { const { error } = await supabase.from('inbox_items').update(updates).eq('id', id); if (error) throw error; };
export const deleteInboxItem = async (item: InboxItem): Promise<void> => { const { error } = await supabase.from('inbox_items').delete().eq('id', item.id); if (error) throw error; const { error: storageError } = await supabase.storage.from('inbox').remove([item.filePath]); if (storageError) console.error('Storage error:', storageError); };
export const uploadFile = async (file: File, bucket: string): Promise<{ path: string }> => { const { data, error } = await supabase.storage.from(bucket).upload(`${Date.now()}_${file.name}`, file); if (error) throw error; return { path: data.path }; };
export const submitApplication = async (appData: { applicationCodeId: string; formData: any; approvalRouteId: string }, applicantId: string): Promise<void> => { const { error } = await supabase.from('applications').insert({ ...appData, applicant_id: applicantId, status: 'pending_approval', submitted_at: new Date() }); if (error) throw error; };
export const updateJobReadyToInvoice = async (jobId: string, value: boolean): Promise<void> => { const { error } = await supabase.from('jobs').update({ ready_to_invoice: value }).eq('id', jobId); if (error) throw error; };
export const createInvoiceFromJobs = async (jobIds: string[]): Promise<Invoice> => { const { data, error } = await supabase.rpc('create_invoice_from_jobs', { job_ids: jobIds }); if (error) throw error; return data; };
export const approveApplication = async (app: ApplicationWithDetails, user: User): Promise<void> => { const { error } = await supabase.rpc('approve_application', { application_id: app.id, approver_id: user.id }); if (error) throw error; };
export const rejectApplication = async (app: ApplicationWithDetails, reason: string, user: User): Promise<void> => { const { error } = await supabase.rpc('reject_application', { application_id: app.id, approver_id: user.id, rejection_reason: reason }); if (error) throw error; };
export const addApprovalRoute = async (route: { name: string; routeData: any }): Promise<void> => { const { error } = await supabase.from('approval_routes').insert(route); if (error) throw error; };
export const updateApprovalRoute = async (id: string, updates: { name?: string; routeData?: any }): Promise<void> => { const { error } = await supabase.from('approval_routes').update(updates).eq('id', id); if (error) throw error; };
export const deleteApprovalRoute = async (id: string): Promise<void> => { const { error } = await supabase.from('approval_routes').delete().eq('id', id); if (error) throw error; };
export const savePostal = async (id: string, patch: Partial<PostalInfo>): Promise<Estimate> => { const { data, error } = await supabase.rpc('update_estimate_postal', { estimate_id: id, postal_patch: patch }); if (error) throw error; return data; };
export const renderPostalLabelSvg = (name: string, company?: string | null): string => `<svg width="300" height="100"><text x="10" y="50">${company || ''} ${name}様</text></svg>`;
export const updateEstimate = async (id: string, patch: Partial<Estimate>): Promise<void> => { const { error } = await supabase.from('estimates').update(patch).eq('id', id); if (error) throw error; };
export const saveTracking = async (id: string, patch: Partial<TrackingInfo>): Promise<void> => { const { error } = await supabase.rpc('update_estimate_tracking', { estimate_id: id, tracking_patch: patch }); if (error) throw error; };
export const addProject = async (project: Partial<Project>, files: { file: File, category: string }[]): Promise<void> => { const { data, error } = await supabase.from('projects').insert(project).select().single(); if (error) throw error; /* File upload logic would go here */ };
export const getAnalysisHistory = async (): Promise<AnalysisHistory[]> => { const { data, error } = await supabase.from('analysis_history').select('*'); if (error) throw error; return data; };
export const addAnalysisHistory = async (history: Partial<AnalysisHistory>): Promise<void> => { const { error } = await supabase.from('analysis_history').insert(history); if (error) throw error; };
export const getActiveAccountItems = async (): Promise<AccountItem[]> => { const { data, error } = await supabase.from('account_items').select('*').eq('is_active', true); if (error) throw error; return data; };